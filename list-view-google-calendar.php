...
class gclv extends gclv_hash_tags{
	...
	var $html_tags = array('li'=>'li', 'p'=>'p', 'dd'=>'dd', 'lip'=>'lip', 'li2'=>'li2', 'li-month'=>'li-month', 'li-title'=>'li-title','li-notitle'=>'li-notitle', 'li-month-notitle'=>'li-month-notitle' ); 
	...
	public function shortcodes($atts){
		...
		if( isset($gc_data['items']) ): 
			foreach($gc_data['items'] as $gc_key=>$gc_value):
				...
				if( isset($time_format) && !empty($time_format) ):
					$date_format_s = explode(" ", $date_format);
					if( count($date_format_s) == 1 ):
				...
				if(!empty($view_end_date) && !empty($end_date_value)):
					if( $start_date_num !== $end_date_num ):
						...
						$start_end_date_value .= $view_end_date . $end_date_time;

/*
						$split_time = explode(" ",$end_date_value);
						array_shift($split_time);
						var_dump($gc_title . " : " . $end_date_num . " : " . $end_date_time);
						$start_end_date_value .= $end_date_time;
						if(!empty($split_time)):
...
*/						
		...
		if(empty($out)):
			if ( isset($settings['google_calendar']['noEventMessage']) && 
			    !empty($settings['google_calendar']['noEventMessage']) ):
				$out = $settings['google_calendar']['noEventMessage'];
			else:
				$out = __('There are no events.', 'list-view-google-calendar');
			endif;
		endif;

...
// Escape at output (context-aware).
...
if ( isset($no_event_link) && !empty($no_event_link) ):
    $out_temp = sprintf(
        ' <li class="%1$s_item"><span class="%1$s_date">%2$s</span> %3$s <span class="%1$s_title" title="%4$s">%5$s</span>',
        $cls,
        $date,
        $cat,
        $title_attr,
    ...
    $out_temp = sprintf(
        ' <li class="%1$s_item"><span class="%1$s_date">%2$s</span> %3$s <a target="_blank" rel="noopener noreferrer" class="%1$s_link" href="%4$s" title="%5$s">%6$s</a>',
        $cls,
        $date,
        $cat,
        $link,
